...
export default async function NotebookDetailPage({
  ...
  const title = titleFromSlug(decodedSlug);
  if (!title) {
    notFound();
  }
    ...
    <div className="mx-auto max-w-6xl px-6 py-10">
      <div className="mb-8 border-b border-slate-200 pb-8">
        ...
        <p className="mt-4 max-w-3xl text-base leading-7 text-slate-600">
          A validated cell manifest and separately isolated output artifacts
          will appear here after the non-executing conversion pipeline is
          proven. This placeholder keeps the read path visible without claiming
          storage, rendering, or execution infrastructure.
        </p>
      ...
      <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_320px]">
        ...
        <aside className="rounded-lg border border-slate-200 bg-white p-5">
          ...
          <p className="mt-2 text-sm leading-6 text-slate-600">
            Cell-level review threads will live here, including top-level
            notebook discussion and threaded replies.
          </p>
          <div className="mt-5 space-y-4">
            <div className="rounded-md border border-slate-200 p-4">
              <p className="text-sm font-medium text-slate-700">Cell 1</p>
              <p className="mt-2 text-sm text-slate-600">
                Confirm the raw dataset checksum before interpreting the first
                summary table.
              </p>
            </div>
            <div className="rounded-md border border-slate-200 p-4">
